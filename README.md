...
- 지도 위 **내일 날씨**
  ...
  - 항목: 최저기온 / 최고기온 / 날씨상태(맑음, 구름많음, 흐림, 비, 눈, 비/눈)
- 표 형태의 **모레 / 글피 기온**
...
```ts
export const MAP_MARKER_POSITIONS = {
  서울: { x: 31.5, y: 21.5 },
  부산: { x: 54.8, y: 70.4 },
  제주: { x: 15.8, y: 92.2 }
};
...
- 기상청 단기예보 API는 전국을 5km x 5km 격자로 제공하므로, 각 도시 위경도를 DFS 격자(`nx`, `ny`)로 변환해서 조회합니다.
- 발표시각은 `02, 05, 08, 11, 14, 17, 20, 23시` 중 가장 최근 것을 자동 선택합니다.
- 강수형태(`PTY`)가 있으면 `비`, `눈`, `비/눈`을 우선 표시하고, 강수가 없을 때만 하늘상태(`SKY`)를 표시합니다.
